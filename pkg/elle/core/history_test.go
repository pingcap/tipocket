...
func TestParseHistory(t *testing.T) {
	...
	assert.Equal(t, history[1], Op{Index: 1, Process: NewOptInt(1), Type: OpTypeOk, Value: &txn1Mops}, "parse history, history[1]")
	assert.Equal(t, history[2], Op{Index: 2, Process: NewOptInt(1), Type: OpTypeInvoke, Value: &[]Mop{Append{Key: "255", Value: 8}, Read{Key: "253", Value: nil}}}, "parse history, history[2]")
	...
	assert.Equal(t, history[4], Op{Index: 4, Process: NewOptInt(2), Type: OpTypeInvoke, Value: &txn3MopsInvoke}, "parse history, history[4]")
