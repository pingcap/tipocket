...
var _ = ginkgo.Describe("etcd", func() {
	...
	ginkgo.It("should tolerant random pod kill chaos", func() {
		...
		cc, err := c.ETCD.ApplyETCD(etcdSpec)
		...
		// start ETCD has to run in env that has etcd binary installed and could access PD address, run it manually is a more feasible idea now
		err = c.ETCD.StartJob(&etcd.ETCDJob{
			ETCD:     cc,
			SinkURI: mysql.URI(),
		}, etcdSpec)
		framework.ExpectNoError(err, "Expected to start a ETCD job.")
